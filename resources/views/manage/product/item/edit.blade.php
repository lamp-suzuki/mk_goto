...
<form action="{{ route('manage.product.item.confirm', ['account' => $sub_domain]) }}" method="post"
  ...
  <div class="form-group">
    ...
    <select name="shop" id="shop" class="form-control @error('shop') is-invalid @enderror" required>
      ...
      <option value="{{ $shop->id }}" @if ($tour->categories_id == $shop->id){{ "selected" }}@endif>{{ $shop->name }}</option>
      @endforeach
      @endif
    </select>
  </div>
  ...
  <div class="form-group">
    ...
    <select name="genre" id="genre" class="form-control @error('genre') is-invalid @enderror" required>
      ...
      <option value="{{ $genre->id }}" @if ($tour->genres_id == $shop->id){{ "selected" }}@endif>{{ $genre->name }}</option>
      @endforeach
      @endif
    </select>
  </div>
  ...
  <div class="form-group">
    ...
    <input type="text" name="name" id="name" class="form-control @error('name') is-invalid @enderror" value="{{ $tour->name }}" required>
  </div>

  <div class="form-group">
    <label for="price">価格</label>
    <div class="input-group">
      <input type="number" id="price" name="price" class="form-control @error('price') is-invalid @enderror" value="{{ $tour->price }}" aria-describedby="yen">
  ...
  <div class="form-group">
    ...
    <textarea name="time" id="time" class="form-control @error('time') is-invalid @enderror" cols="30" rows="10" required>{!! e($tour->time) !!}</textarea>
  </div>

  <div class="form-group">
    <label for="explanation">ツアー概要</label>
    <textarea name="explanation" id="explanation" class="form-control @error('explanation') is-invalid @enderror" cols="30" rows="10" required>{!! e($tour->explanation) !!}</textarea>
  </div>
